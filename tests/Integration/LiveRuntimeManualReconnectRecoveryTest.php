...
final class LiveRuntimeManualReconnectRecoveryTest extends AsyncTestCase
{
    public function testOptInManualLiveReconnectRecoveryAndCleanShutdown(): void
    {
        ...
        if (getenv('ESL_REACT_LIVE_MANUAL_RECONNECT_TEST') !== '1') {
            self::markTestSkipped('Set ESL_REACT_LIVE_MANUAL_RECONNECT_TEST=1 to run the manual live reconnect harness.');
        }

        $host = getenv('ESL_REACT_LIVE_HOST');
        if (!is_string($host) || $host === '') {
            self::markTestSkipped('ESL_REACT_LIVE_HOST is required for the manual live reconnect harness.');
        }
    ...
    {
        fwrite(STDOUT, sprintf("[manual reconnect] %s\n", $message));
        fflush(STDOUT);
    }
